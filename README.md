...
- UI는 재사용 가능한 `component + props` 구조를 기본으로 작성
- 반복되는 UI나 섹션별 표현은 컴포넌트로 분리
- 하드코딩을 늘리기보다 의미 있는 props와 데이터 구조로 관리

### 5. Responsive Design

- 모든 화면은 `데스크탑`과 `모바일`을 함께 고려해서 만들기
...
- 단, 현재 UX와 성능을 해치지 않는 범위에서 적용

### 7. Typography

- 텍스트 일관성은 매우 중요
- 사용자가 별도로 지정하지 않는 한 모든 텍스트는 기본적으로 `Noto Sans KR`를 사용
- 새로운 폰트를 추가할 때는 명시적인 이유가 있어야 함
